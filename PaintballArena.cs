...
namespace Oxide.Plugins
{
    ...
    public class PaintballArena : RustPlugin
    {
        ...
        private void OnEntityEnter(TriggerBase trigger, BaseEntity entity)
        {
            ...
            var baseEntity = trigger.GetComponent<BaseEntity>();
            if (baseEntity == null) return;
            if (!triggerLookup.TryGetValue(baseEntity.net.ID, out var definition)) return;
        ...
        private void OnPlayerInput(BasePlayer player, InputState input)
        {
            ...
            if (Volatile.Read(ref session.MovementLocks) <= 0) return; // Movement locks only apply when count is above zero.
